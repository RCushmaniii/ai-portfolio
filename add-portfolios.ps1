...
foreach ($repo in $repos) {
    ...
    if (!(Test-Path $sourceFile)) {
        Write-Host "   Source file not found: $($repo.file)" -ForegroundColor Red
        continue
    }
    ...
    # Copy PORTFOLIO.md
    Write-Host "   Copying PORTFOLIO.md..." -ForegroundColor Gray
    Copy-Item $sourceFile -Destination "PORTFOLIO.md" -Force

    # Check if file was copied
    if (!(Test-Path "PORTFOLIO.md")) {
        Write-Host "   Failed to copy PORTFOLIO.md" -ForegroundColor Red
        continue
    }
    ...
    if (!$status) {
        Write-Host "    No changes (PORTFOLIO.md already up to date)" -ForegroundColor Gray
        continue
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "   Successfully added PORTFOLIO.md" -ForegroundColor Green
    } else {
